...
void local_transform_system(ecs::layer_view lv, entt::entity entity, time_point time_point [[maybe_unused]]) {
    auto* maybe_local_tf = lv.registry.template try_get<local_transform>(entity);
    if (maybe_local_tf == nullptr) {
        DEBUG_ASSERT(
            !lv.registry.template all_of<transform>(entity),
            "all transforms have to have local_transform component in tree structure"
        );
        return;
    }
    meta::dirty<transform>& local_tf = *maybe_local_tf;
    node& node_component = lv.registry.template get<node>(entity);
    local_tf.release()
        .map([&](const transform& changed_local_tf) -> transform {
            ...
            const transform* maybe_parent_tf = lv.registry.template try_get<transform>(parent);
        ...
        .map([&](transform new_tf) {
            lv.registry.template emplace_or_replace<transform>(entity, new_tf);

            for (entt::entity child_entity : node_component.children) {
                local_transform* maybe_child_local_tf = lv.registry.template try_get<local_transform>(child_entity);
...
void local_transform_system(ecs::layer_view lv, time_point time_point) {
    tree_update_system(tree_update_order::TOP_DOWN, lv, detail::local_transform_system, time_point);
}
