...
struct material {
    meta::persistent<game::texture> diffuse;
    meta::persistent<game::texture> specular;
    meta::persistent<game::texture> emission;
    float shininess;
};
...
exec::async<game::shader> create_object_shader(const script::example_context& example_ctx) {
    const std::array<gfx::shader, 2> shaders{
        *ASSERT_VAL(gfx::shader::load_from_file(
            gfx::shader_type::vertex, example_ctx.examples_path / "shaders/03_lightmap.vert"
        )),
        *ASSERT_VAL(gfx::shader::load_from_file(
            gfx::shader_type::fragment, example_ctx.examples_path / "shaders/03_lightmap.frag"
        )),
    };
    ...
    auto set_time = *ASSERT_VAL(sp_bind.make_uniform_setter(glUniform1f, "u_time"));
        ...
        .setup{ [ //
            ...
            return [&](const gfx::bound_vertex_array& bound_va,
                ...
                for (const entt::entity entity : entities) {
                    ...
                    const auto bound_emission = mat.emission->tex.activate(2);
...
exec::async<
    std::tuple<entt::entity, ecs::resource<game::shader>&, ecs::resource<game::vertex>&, ecs::resource<game::texture>&>>
    ...
    auto& texture_resource = layer.registry.emplace<ecs::resource<game::texture>::ptr_type>(
        entity, ecs::resource<game::texture>::make(*e_ctx.sync_exec)
    );
    ...
    layer.registry.emplace<game::update>(
        entity,
        [&](ecs::layer& layer, entt::entity entity, game::time_point time_point) {
    ...
    co_return std::tuple //
        <entt::entity, ecs::resource<game::shader>&, ecs::resource<game::vertex>&, ecs::resource<game::texture>&>{
            entity,
            *shader_resource,
            *vertex_resource,
            *texture_resource,
...
exec::async<std::vector<entt::entity>>
    ...
    auto& tex_resource = layer.registry.get<ecs::resource<game::texture>::ptr_type>(layer.root);
    meta::persistent<game::texture> tex_diffuse =
        *ASSERT_VAL(tex_resource->lookup_unsafe("texture.diffuse"_us(example_ctx.uss)));
    meta::persistent<game::texture> tex_specular =
        *ASSERT_VAL(tex_resource->lookup_unsafe("texture.specular"_us(example_ctx.uss)));
    meta::persistent<game::texture> tex_emission =
        *ASSERT_VAL(tex_resource->lookup_unsafe("texture.emission"_us(example_ctx.uss)));
...
exec::async<void> create_scene(
    ...
    auto [global_entity, shader_resource, vertex_resource, texture_resource] =
        co_await create_global_entity(e_ctx, layer);
    // common vvv
    std::ignore =
    ...
    ASSERT(co_await texture_resource.require(
    ...
    ASSERT(co_await texture_resource.require(
        "texture.emission"_us(example_ctx.uss),
        script::create_texture(example_ctx.examples_path / "textures/03_lightmap_emission.jpg")
    ));
    // common ^^^
...
void main(int argc, char** argv) {
    ...
    auto w_ctx = *ASSERT_VAL(
        game::window_context::initialize(sl::meta::null, "03_lightmap", { 1280, 720 }, { 0.1f, 0.1f, 0.1f, 0.1f })
    );
