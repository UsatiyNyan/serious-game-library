...
struct node {
    ...
    static void attach_child(
        ...
        entt::registry& registry = lv.registry;
    ...
    static void attach_child(ecs::layer_view lv, entt::entity parent_entity, entt::entity child_entity) {
        node& parent_node = lv.registry.get_or_emplace<node>(parent_entity);
        node& child_node = lv.registry.get_or_emplace<node>(child_entity);
        attach_child_impl(parent_entity, parent_node, child_entity, child_node);
    }
...
using update = meta::unique_function<void(ecs::layer_view, entt::entity, time_point)>;
