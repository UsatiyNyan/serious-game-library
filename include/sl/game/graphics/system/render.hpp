...
concept GfxLayerRequirements = GameLayerRequirements<Layer> && requires(Layer& layer) {
    { layer.world } -> std::convertible_to<basis>;
...
void graphics_system(Layer& layer, const window_frame& window_frame) {
    ...
    for (const auto& [camera_entity, camera_component, camera_tf] : camera_entities.each()) {
        const auto camera_frame = window_frame.for_camera(layer.world, camera_component, camera_tf);
